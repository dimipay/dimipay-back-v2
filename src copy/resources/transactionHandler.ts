...
export const approvePrepaidCard = async (paymentMethod: TransactionPaymentMethod, approveAmount: number) => {
...
export const approveGeneralCard = (paymentMethod: TransactionPaymentMethod, approveAmount: number) => {
    if(paymentMethod.type !== "GENERAL") {
        throw new Error("일반카드가 아닙니다.");
    }
}

export const chargePrepaidCard = async (paymentMethod: TransactionPaymentMethod, chargeAmount: number, method: string) => {
    if(paymentMethod.type !== "PREPAID") {
        throw new Error("선불카드가 아닙니다.");
    }
